...
itself. -/
theorem threeRoute_rightInv_leftStarSurvivor (S : ResolvedThreeRouteFullSupply D G imageOf)
    ...
    (honeRecover : ∀ k : OneStageStarIndex D G s, S.oneStarRecover s k.toStarVertex = k)
  ...
  have hO : ¬ isContractSurvivingVertex s.1.1 w.1 :=
    fun hOs => S.originalSurvivor_not_leftStar s hOs i hL hvw.symm
  have hEx := (S.twoStageSurvivor_cases s
  ...
  have hinv : (threeRouteCorrInvFun S.toResolvedThreeRouteInvFunSupply s w).1
    ...
    threeRouteCorrInvFun_survivorLeft_val S.toResolvedThreeRouteInvFunSupply s w hnotstar hO
  ...
  have hrec : S.oneStarRecover s
      ⟨(threeRouteCorrInvFun S.toResolvedThreeRouteInvFunSupply s w).1, hfwdstar⟩
      = Classical.choose hEx := by
    have he : (⟨(threeRouteCorrInvFun S.toResolvedThreeRouteInvFunSupply s w).1, hfwdstar⟩ :
        {v : VertexId // isContractStarVertex s.1.1 (D.starOf G s.1.1) v})
        = (Classical.choose hEx).toStarVertex := Subtype.ext hinv
    rw [he, honeRecover]
  have hfwdleft : (S.oneStarRecover s
      ⟨(threeRouteCorrInvFun S.toResolvedThreeRouteInvFunSupply s w).1, hfwdstar⟩).isLeft := by
    rw [hrec]; exact hspec.1
  -- forward keeps the vertex; chosen index's vertex is w.1
  apply Subtype.ext
  rw [threeRouteCorrToFun_leftStar_val S.toResolvedThreeRouteToFunSupply s _ hfwdstar hfwdleft, hinv]
  exact hspec.2
