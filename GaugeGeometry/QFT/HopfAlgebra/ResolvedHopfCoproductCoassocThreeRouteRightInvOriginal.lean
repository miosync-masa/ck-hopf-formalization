...
itself. -/
theorem threeRoute_rightInv_originalSurvivor (S : ResolvedThreeRouteInverseLawSupply D G imageOf)
  ...
  have hinv : (threeRouteCorrInvFun S.route.toResolvedThreeRouteInvFunSupply s w).1 = w.1 :=
    threeRouteCorrInvFun_survivorOriginal_val S.route.toResolvedThreeRouteInvFunSupply s w hnotstarB hOrig
  -- inverse output is an s.1.1 survivor, hence not an s.1.1 star
  have hOrig' : isContractSurvivingVertex s.1.1
      (threeRouteCorrInvFun S.route.toResolvedThreeRouteInvFunSupply s w).1 := by rw [hinv]; exact hOrig
  have hnotstarA : ¬ isContractStarVertex s.1.1 (D.starOf G s.1.1)
      (threeRouteCorrInvFun S.route.toResolvedThreeRouteInvFunSupply s w).1 :=
    fun hst => contract_surviving_not_star s.1.1 _ hfreshA hOrig' hst
  -- forward keeps the vertex
  apply Subtype.ext
  rw [threeRouteCorrToFun_survivor_val S.route.toResolvedThreeRouteToFunSupply s _ hnotstarA, hinv]
